#1) A partir de una lista vacía, utilizar un ciclo while para cargar 
# allí números negativos del -15 al -1
lis=[]
n=-15
while n<0:
    lis.append(n)
    n+=1
print(lis)
#2) ¿Con un ciclo while sería posible recorrer la lista para imprimir sólo los números pares?
n=0
lis1=[]
while n<(len(lis)):
    if lis[n]%2 == 0:
        lis1.append(lis[n])
    n+=1
print(lis1)
#3) Resolver el punto anterior sin utilizar un ciclo while
lis1=[]
n=0
for n in range(len(lis)):
    if lis[n] % 2 == 0:
        lis1.append(lis[n])
    n+=1
print(lis1)
#4) Utilizar el iterable para recorrer sólo los primeros 3 elementos
n=0
while n<=2:
    print(lis[n])
    n+=1

#5) Utilizar la función **enumerate** para obtener dentro del iterable, tambien el índice al que 
# corresponde el elemento
for i,c in enumerate(lis):
    print(i,c)

#6) Dada la siguiente lista de números enteros entre 1 y 20, crear un ciclo donde se completen 
# los valores faltantes: lista = [1,2,5,7,8,10,13,14,15,17,20]
lista=[1,2,5,7,8,10,13,14,15,17,20]
n=1
while n< 20:
    if n not in lista:
        lista.insert(n-1,n)
    n+=1
print(lista)


#7) La sucesión de Fibonacci es un listado de números que sigue la fórmula: <br>
'''n<sub>0</sub> = 0<br>
n<sub>1</sub> = 1<br>
n<sub>i</sub> = n<sub>i-1</sub> + n<sub>i-2</sub><br>
Crear una lista con los primeros treinta números de la sucesión.<br>
'''
lista=[0,1]
n=2
while n < 30:
    lista.append(lista[n-2]+ lista[n-1])
    n+=1
print(lista)

#8) Realizar la suma de todos elementos de la lista del punto anterior
print(sum(lista))


#9) La proporción aurea se expresa con una proporción matemática que nace el número irracional Phi= 1,618… 
# que los griegos llamaron número áureo. El cuál se puede aproximar con la sucesión de Fibonacci. 
# Con la lista del ejercicio anterior, imprimir el cociente de los últimos 5 pares de dos números contiguos:<br>
'''Donde i es la cantidad total de elementos<br>
n<sub>i-1</sub> / n<sub>i</sub><br>
n<sub>i-2</sub> / n<sub>i-1</sub><br>
n<sub>i-3</sub> / n<sub>i-2</sub><br>
n<sub>i-4</sub> / n<sub>i-3</sub><br>
n<sub>i-5</sub> / n<sub>i-4</sub><br>
''' 
n=29
while n >19:
    print(lista[n]/lista[n-1])
    n-=2



#10) A partir de la variable cadena ya dada, mostrar en qué posiciones aparece la letra "n"<br>
cadena = 'Hola Mundo. Esto es una practica del lenguaje de programación Python'
for indice,letra in enumerate(cadena):
    if letra == 'n':
        print(indice)



#11) Crear un diccionario e imprimir sus claves utilizando un iterador
dicc={'practicas':('anapana','vipassana'),'sitios':('Cali','Medellin')}
for i in dicc:
    print(i)


#12) Convertir en una lista la variable "cadena" del punto 10 y luego recorrerla con un iterador 
cadena= list(cadena)
for i in cadena:
    print(i)
    

#13) Crear dos listas y unirlas en una tupla utilizando la función zip
a=[1,2,3]
b=[4,5,6]
c=zip(a,b)
print(list(c))


#14) A partir de la siguiente lista de números, crear una nueva sólo si el número es divisible por 7<br>
lis = [18,21,29,32,35,42,56,60,63,71,84,90,91,100]
lis1=[]
for numero in lis:
    if numero % 7 == 0:
        lis1.append(numero)
print(lis1)
lis2=[numero for numero in lis if numero % 7 == 0]
print(lis2)

#15) A partir de la lista de a continuación, contar la cantidad total de elementos que contiene, 
# teniendo en cuenta que un elemento de la lista podría ser otra lista:<br>
lis = [[1,2,3,4],'rojo','verde',[True,False,False],['uno','dos','tres']]
n=0
for elemento in lis:
    n+=1
print('Los elementos en la lista son:',n)

#16) Tomar la lista del punto anterior y convertir cada elemento en una lista si no lo es
for indice, elemento in enumerate(lis):
    if (type(elemento) != list):
        lis[indice]=[elemento]
print(lis)